...
<tbody>
	<?php $c = 1; ?>
	<?php $__currentLoopData = $ranks; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $k => $v): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
	<?php $ur = "quiz/results/seenX/".$id."/" . $k; ?>
	<tr>
		<td><?php echo e($c); ?></td>
		<td><?php echo e(\App\User::find($k)->name); ?> ( <?php echo e(\App\User::find($k)->email); ?> )</td>
		<td><?php echo e($v); ?> / <?php echo e($answers); ?> </td>
		<td><button class="btn btn-primary btn-sm viewResultx" data-toggle="modal" route="<?php echo e(url($ur)); ?>" data-target="#userModalQuizResults">View Results</button></td>
	</tr>
	<?php $c++; ?>
...
?>
<?php if($qf == 0): ?>
<button route="<?php echo e(route('quiz.publish.results', $id)); ?>" id="publishResx" class="btn btn-success btn-sm"><i class="fa fa-check"></i> Publish Results Now</button>

<button onclick="$('#printResultX').css('display', ''); printJS('printResultX', 'html'); $('#printResultX').css('display', 'none');"  class="btn btn-danger btn-sm"><i class="fa fa-print"></i> Print Results</button>
<?php else: ?>
<button route="<?php echo e(route('quiz.unpublish.results', $id)); ?>" id="unpublishResx" class="btn btn-success btn-sm"><i class="fa fa-ban"></i> UnPublish Results Now</button>
	  ...
	  showLoaderOnConfirm: true
		...
		alert(route);
	  	Biggo.talkToServer(route, data).then(function(res){
	  		//return;
			//window.location = '<?php echo e(route("quiz.refresh")); ?>';
		});
	});
});
